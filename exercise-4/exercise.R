...
# and get some basic information about the number of rows/column
View(flights)
...
sorted.flights <- arrange(flights, -time_gained)

# Try doing the last 2 steps in a single operation using the pipe operator
flights <- mutate(flights, time_gained = arr_delay - dep_delay) %>% arrange(desc(time_gained))

# Make a histogram of the amount of gain using the `hist` command
hist(flights$time_gained)

# On average, did flights gain or lose time?
mean(flights$time_gained, na.rm = TRUE) #flights lost time

# Create a data.frame that is of flights headed to seatac ('SEA'), 
sea.tac <- filter(flights, dest == "SEA")

# On average, did flights to seatac gain or loose time?
mean(sea.tac$time_gained, na.rm = TRUE) #lose
